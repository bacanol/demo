...
#
#JIRA_HOME=/opt/jira_dc_node1/jira_data
#export JAVA_HOME=/opt/jira_dc_node1/current/jdk
#export JRE_HOME=/opt/jira_dc_node1/current/jre
export SDK_HOME=$JAVA_HOME
export JDK_HOME=$JAVA_HOME
umask 0027
JVM_JMX_ARGS=" -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=8880 -Dcom.sun.management.jmxremote.local.only=false -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false "
JVM_PROXY_ARGS=""
### Changed by nho due to non deterministic stops in JIRA data center production 20170912
#JVM_GC_ARGS="-XX:+AggressiveOpts -XX:+UseBiasedLocking -XX:+UseConcMarkSweepGC -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=75 -XX:+ScavengeBeforeFullGC -XX:TargetSurvivorRatio=80 -XX:SurvivorRatio=6 -XX:+UseBiasedLocking -XX:MaxTenuringThreshold=15 -XX:ParallelGCThreads=8 -XX:+UseCompressedOops -XX:+OptimizeStringConcat -XX:NewRatio=3 -XX:MaxMetaspaceSize=768m -XX:+UseCodeCacheFlushing"
JVM_GC_ARGS="-XX:+UseG1GC -XX:MaxPermSize=1G -XX:ConcGCThreads=4 -XX:ParallelGCThreads=16 -XX:MetaspaceSize=768m -XX:MaxMetaspaceSize=1G -XX:MaxGCPauseMillis=100 -XX:G1HeapWastePercent=10 -XX:G1ReservePercent=20 -XX:+UnlockExperimentalVMOptions -XX:G1NewSizePercent=5 -XX:G1MaxNewSizePercent=60"
JVM_CATWORKX_ARGS="-Dfile.encoding=UTF-8  -Dmail.mime.multipart.ignoreexistingboundaryparameter=true  -Duser.language=de -Duser.country=DE "

#
#  Occasionally Atlassian Support may recommend that you set some specific JVM arguments.  You can use this variable below to do that.
#
JVM_SUPPORT_RECOMMENDED_ARGS=" $JVM_JMX_ARGS $JVM_PROXY_ARGS $JVM_GC_ARGS $JVM_CATWORKX_ARGS "
